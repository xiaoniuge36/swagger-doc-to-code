...
export class OpenAPIV3Parser extends BaseParser {
  ...
  parseResponse(
    responses: OpenAPIV3.ResponsesObject
  ): TreeInterfacePropertiesItem | string | undefined {
    ...
    const responseBodySchema = this.dereferenceSchema(
  ...
  parseArray(
    ...
    if (arrayItem.itemsRequiredNamesList) {
      itemSchema.required = arrayItem.itemsRequiredNamesList.includes(
        arrayItem.name
      );
    }
