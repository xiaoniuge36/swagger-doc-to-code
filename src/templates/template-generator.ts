...
export class TemplateGenerator {
   ...
   */
  static generateEnhancedTemplate(): string {
    return `/**
...
\}`;
   ...
   */
  static generateBasicTemplate(): string {
   ...
   */
  static getTemplateHeader(): string {
    return `/**
 * Swagger Doc To Code 模板配置文件
 * 此文件用于自定义生成的 TypeScript 接口代码格式
 * 更多高级用法请参考: https://github.com/xiaoniuge36/swagger-doc-to-code
 */`;
   ...
   */
  static getNamespaceFunction(): string {
    return `/**
 * 自定义命名空间名称
 * @param {Object} params - 接口参数
 * @param {string} params.groupName - 分组名称
...
function namespace(params) {
   ...
   */
  static getParamsFunction(): string {
    return `/**
 * 自定义参数接口
 * @param {Object} params - 接口上下文
 * @returns {string} 参数接口代码
...
\}\`
   ...
   */
  static getResponseFunction(): string {
    return `/**
 * 自定义响应接口
 * @param {Object} params - 接口上下文
 * @returns {string} 响应接口代码
...
\}\`
   ...
   */
  static getCopyRequestFunction(): string {
    return `/**
 * 复制请求函数模板
 * 优化版本：生成单个请求函数，包含完整注释和类型定义
 * @param {Object} fileInfo - 文件信息
